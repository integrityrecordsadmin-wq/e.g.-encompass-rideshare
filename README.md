...
- **Vehicle age rule table (`VEHICLE_AGE_LIMIT_YEARS_BY_STATE` in both driver pages) is a starting point, not verified.** Real rules vary by state and city, change over time, and sources sometimes disagree even for the same state. Confirm the actual current requirement for every state/city you operate in before relying on this for real approvals.
