...
import {
  ...
  createFamily, joinFamily, subscribeToFamily, leaveFamily, getFamilyMembers,
} from "../../lib/db";
      ...
      <div className="px-6 space-y-2 flex-1 overflow-y-auto">
        {members.map((m) => (
          <div key={m.uid} className="flex items-center gap-3 rounded-xl p-3"
            ...
            <p className="text-sm font-medium" style={{ color: "#F5F5F0" }}>{m.name}</p>
            {m.uid === person.uid && (
              <span className="ml-auto text-xs" style={{ color: "#7A7F8A" }}>You</span>
            )}
          </div>
        ))}
