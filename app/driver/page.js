...
function DriverAuthScreen({ onAuthed }) {
  ...
  const submitVehicleInfo = async (e) => {
    ...
    setBusy(true);
    try {
      const driver = pending.source === "google"
        ? await completeDriverGoogleSignup(pending.uid, { name, email: pending.email, carModel, plate, vehicleType })
        : await completeDriverMagicLinkSignup(pending.uid, { name, email: pending.email, carModel, plate, vehicleType });
      onAuthed(driver);
    } catch (err) {
