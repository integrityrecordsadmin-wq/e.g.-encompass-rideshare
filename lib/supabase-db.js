...
export async function createRoundTripRide(rideData) {
  const { roundTripType, returnTime, ...baseRide } = rideData;

  if (roundTripType === 'same_driver') {
    const { data, error } = await supabase
      .from('rides')
      .insert({
        ...baseRide,
        is_round_trip: true,
        round_trip_type: 'same_driver',
  ...
  if (roundTripType === 'separate_rides') {
    const { data: outbound, error: outErr } = await supabase
      .from('rides')
      .insert({ ...baseRide, is_round_trip: true, round_trip_type: 'separate_rides' })
      .select()
      .single();
    if (outErr) throw outErr;

    const { data: returnRide, error: retErr } = await supabase
      .from('rides')
      .insert({
        ...baseRide,
        pickup_location: baseRide.dropoff_location,
        pickup_address: baseRide.dropoff_address,
        dropoff_location: baseRide.pickup_location,
        dropoff_address: baseRide.pickup_address,
        scheduled_time: returnTime,
        is_round_trip: true,
        round_trip_type: 'separate_rides',
        linked_ride_id: outbound.id,
      })
      .select()
      .single();
    if (retErr) throw retErr;

    await supabase.from('rides').update({ linked_ride_id: returnRide.id }).eq('id', outbound.id);

    return { outbound, returnRide };
  }
}
...
export async function checkFlatrateEligibility(riderId, vehicleType) {
  if (vehicleType !== 'standard') return { eligible: false, reason: 'plan_only_covers_standard' };

  const plan = await getRiderFlatratePlan(riderId);
  if (!plan) return { eligible: false, reason: 'no_active_plan' };

  const today = new Date();
  const dayNames = ['sun','mon','tue','wed','thu','fri','sat'];
  const todayName = dayNames[today.getDay()];
  if (!plan.workdays.includes(todayName)) {
    return { eligible: false, reason: 'not_a_scheduled_workday' };
  }
